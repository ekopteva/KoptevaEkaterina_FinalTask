...
Процедура ЗаполнитьНаСервере()
	...
	|ИЗ
	|	РегистрСведений.BKM_УсловияОплатыСотрудников.СрезПоследних(&НачалоПериода,) КАК
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЕСТЬNULL(BKM_УсловияОплатыСотрудниковСрезПоследних.ГрафикРабот,
	|		ЗНАЧЕНИЕ(Справочник.BKM_ГрафикиРабот.ПустаяСсылка)) КАК ГрафикРабот,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.BKM_ВыполненныеСотрудникомРаботы.ОстаткиИОбороты(&НачалоПериода, &КонецПериода,,,
	|			Сотрудник В
	|			(ВЫБРАТЬ
	|				ВТ_Сотрудники.Сотрудник КАК Сотрудник
	|			ИЗ
...
Функция ДатыЗаполненыВерно()
	...
	Для Каждого СтрокаТЧ Из Объект.ОсновныеНачисления Цикл
		...
		Если   СтрокаТЧ.ДатаНачала > СтрокаТЧ.ДатаОкончания Тогда 
			...
			Результат = Ложь;
			ТекстСообщения = "Неправильно заполнены поля ДатаНачала и ДатаОкончания. ДатаНачала больше чем ДатаОкончания";
