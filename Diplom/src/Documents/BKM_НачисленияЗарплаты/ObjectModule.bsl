...
Процедура СформироватьДвиженияПоОсновнымНачислениям()
	...
	|	И BKM_НачисленияЗарПлатыОсновныеНачисления.ВидРасчета = &Оклад
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.BKM_УсловияОплатыСотрудников.СрезПоследних(&ДатаДокумента, Сотрудник В
	|			(ВЫБРАТЬ
	|				BKM_НачисленияЗарплатыОсновныеНачисления.Сотрудник КАК Сотрудник
	|			ИЗ
	|				Документ.BKM_НачисленияЗарплаты.ОсновныеНачисления КАК BKM_НачисленияЗарплатыОсновныеНачисления
	|			ГДЕ
	|				BKM_НачисленияЗарплатыОсновныеНачисления.Ссылка = &Ссылка
	|				И BKM_НачисленияЗарплатыОсновныеНачисления.ВидРасчета = &Оклад)) КАК BKM_УсловияОплатыСотрудниковСрезПоследних
	...
	|	BKM_НачисленияЗарплатыОсновныеНачисления.ДатаНачала КАК ПериодДействияНачало,
	|	BKM_НачисленияЗарплатыОсновныеНачисления.ДатаОкончания КАК ПериодДействияКонец,
	|	BKM_НачисленияЗарплатыОсновныеНачисления.ГрафикРабот КАК ГрафикРабот,
	|	ВЫБОР
	|		КОГДА BKM_НачисленияЗарплатыОсновныеНачисления.ВидРасчета = ЗНАЧЕНИЕ(ПланВидовРасчета.BKM_ОсновныеНачисления.Отпуск)
	|			ТОГДА НАЧАЛОПЕРИОДА(ДОБАВИТЬКДАТЕ(BKM_НачисленияЗарплатыОсновныеНачисления.ДатаНачала, МЕСЯЦ, -12), МЕСЯЦ)
	|		ИНАЧЕ ДАТАВРЕМЯ(1, 1, 1)
	|	КОНЕЦ КАК БазовыйПериодНачало,
	|	ВЫБОР
	|		КОГДА BKM_НачисленияЗарплатыОсновныеНачисления.ВидРасчета = ЗНАЧЕНИЕ(ПланВидовРасчета.BKM_ОсновныеНачисления.Отпуск)
	|			ТОГДА КОНЕЦПЕРИОДА(ДОБАВИТЬКДАТЕ(BKM_НачисленияЗарплатыОсновныеНачисления.ДатаНачала, МЕСЯЦ, -1), МЕСЯЦ)
	|		ИНАЧЕ ДАТАВРЕМЯ(1, 1, 1)
	|	КОНЕЦ КАК БазовыйПериодКонец,
	|	BKM_НачисленияЗарплатыОсновныеНачисления.Ссылка.Дата КАК ПериодРегистрации,
	|	ЕСТЬNULL(ВТ_Оклады.Оклад, 0) КАК Показатель
	|ИЗ
	|	Документ.BKM_НачисленияЗарплаты.ОсновныеНачисления КАК BKM_НачисленияЗарплатыОсновныеНачисления
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Оклады КАК ВТ_Оклады
	|		ПО BKM_НачисленияЗарплатыОсновныеНачисления.Сотрудник = ВТ_Оклады.Сотрудник
	|ГДЕ
	|	BKM_НачисленияЗарплатыОсновныеНачисления.Ссылка = &Ссылка";
	...
	Пока Выборка.Следующий() Цикл 
		...
		Если Выборка.ВидРасчета = ПланыВидовРасчета.BKM_ОсновныеНачисления.Оклад И Выборка.Показатель = 0 Тогда
...
Процедура РассчитатьОклады() 
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	РегистрРасчета.BKM_ОсновныеНачисления.ДанныеГрафика(Регистратор = &Регистратор
	...
	Пока Выборка.Следующий() Цикл
		...
		// Движения по регистру BKM_ОсновныеНачисления
...
Процедура РассчитатьОтпускныеНачисления()
	...
	Запрос.Текст = 
		...
		|	ЕСТЬNULL(BKM_ОсновныеНачисленияДанныеГрафика.ЗначениеФактическийПериодДействия, 0) КАК Факт,
		|	ЕСТЬNULL(BKM_ОсновныеНачисленияБазаBKM_ОсновныеНачисления.РезультатБаза, 0) +
		|		ЕСТЬNULL(BKM_ОсновныеНачисленияБазаBKM_ДополнительныеНачисления.РезультатБаза, 0) КАК РезультатБаза,
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрРасчета.BKM_ОсновныеНачисления.БазаBKM_ДополнительныеНачисления(&Измерения, &Измерения,,
		|			Регистратор = &Регистратор
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрРасчета.BKM_ОсновныеНачисления.ДанныеГрафика(Регистратор = &Регистратор
...
Процедура РассчитатьНДФЛ()
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	РегистрРасчета.BKM_Удержания.БазаBKM_ОсновныеНачисления(&Измерения, &Измерения, &Разрезы, Регистратор = &Ссылка
		...
		|ИЗ
		|	РегистрРасчета.BKM_Удержания.БазаBKM_ДополнительныеНачисления(&Измерения, &Измерения, &Разрезы, Регистратор = &Ссылка
...
Процедура СформироваДвиженияВыполненныеСотрудникомРаботы(Отказ)
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	BKM_НачисленияЗарплатыДополнительныеНачисления.Сотрудник КАК Сотрудник,
		|	СУММА(BKM_НачисленияЗарплатыДополнительныеНачисления.СуммаНачисления) КАК СуммаНачисления,
		|	СУММА(BKM_НачисленияЗарплатыДополнительныеНачисления.НачисленоЧасов) КАК НачисленоЧасов
		|ПОМЕСТИТЬ ВТ_Сотрудник
		|ИЗ
		|	Документ.BKM_НачисленияЗарплаты.ДополнительныеНачисления КАК
		|		BKM_НачисленияЗарплатыДополнительныеНачисления
		|ГДЕ
		|	BKM_НачисленияЗарплатыДополнительныеНачисления.Ссылка = &Ссылка
		|СГРУППИРОВАТЬ ПО
		|	BKM_НачисленияЗарплатыДополнительныеНачисления.Сотрудник
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Сотрудник
		|;
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.BKM_ВыполненныеСотрудникомРаботы.Остатки(&МоментВремени, Сотрудник В
		|			(ВЫБРАТЬ
		|				ВТ_Сотрудник.Сотрудник КАК Сотрудник
		|			ИЗ
