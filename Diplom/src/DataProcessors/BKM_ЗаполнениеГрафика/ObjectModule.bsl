Процедура ЗаполнитьГрафик(ДатаНачала, ДатаОкончания, ВыходныеДни, ГрафикРаботы) Экспорт 
	...
	Набор = РегистрыСведений.BKM_ГрафикиРабот.СоздатьНаборЗаписей();
	Набор.Отбор.ГрафикРабот.Установить(ГрафикРаботы);
	Набор.Прочитать();
	...
	Для к = 0 По Набор.Количество()-1 Цикл
		...
		Иначе
			Пока Дат < Мин(Запись.Дата, ДатаОкончания) Цикл
				...
				НоваяЗапись.ГрафикРабот = ГрафикРаботы;
				Если Найти(ВыходныеДни, Строка(ДеньНедели(Дат))) Тогда
	...
	Пока Дат <= ДатаОкончания Цикл
		...
		Запись.Дата = Дат; 
		Запись.ГрафикРабот = ГрафикРаботы;
		Если Найти(ВыходныеДни, Строка(ДеньНедели(Дат))) Тогда
			Запись.Значение = 0;
		Иначе	          
			Запись.Значение = 1;
